...
<?php
	...
	if($d1<date('d-m-Y', strtotime(date("d-m-Y"))))
	#if((strcmp($d1,date('d-m-Y', strtotime(date("d-m-Y")))))) 
	{
		echo "<script>alert('CheckIN date in smaller than current date.');window.history.back();</script>";
		exit();
	}
