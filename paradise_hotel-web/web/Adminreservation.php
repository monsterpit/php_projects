	...
	<div class="header">
		...
		<div class="h_right">
			...
			<ul class="menu">
				...
				<li class="active"><a href="Adminreservation.php">reservation</a></li> |
					...
					<td>
						<div id="info"  >
							<table style="width:900px;">
								<tr style="color:#000000;font-weight:bold;text-align:center;"><td>id</td><td>username</td><td>email</td><td>contact</td><td>roomtype</td><td> paymentmethod</td><td>cardnumber</td><td>cvc</td><td>expiresdate</td><td>totalamount</td><td>totaldays</td><td>paid</td><td>action</td></tr> 
								<?php
									...
									if (mysqli_num_rows($result) > 0) {
										...
										while($row = mysqli_fetch_assoc($result)) {
											echo "<tr><td>{$row["id"]}</td><td>{$row["username"]}</td><td>{$row["email"]}</td><td>{$row["contact"]}</td><td>{$row["roomtype"]}</td><td>{$row["paymentmethod"]}</td><td>{$row["cardnumber"]}</td><td>{$row["cvc"]}</td><td>{$row["expiresdate"]}</td><td>{$row["totalamount"]}</td><td>{$row["totaldays"]}</td><td>{$row["paid"]}</td><td><a href='DeleteRecord.php?id={$row["id"]}'>Delete</a></td></tr>";
										}
									}
									else {
										echo "<tr><td colspan='4'>No Rooms booked.</td></tr>";
									}
